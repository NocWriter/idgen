...
public class SecureRandomIdGenerator implements IdGenerator {
    ...
    /**
     * Default size of token, in digits.
     */
    public static final int DEFAULT_TOKEN_SIZE = 24;
    ...
    public String generate() {
        ...
        //
        int numberOfBytes = (int) (Math.round(Math.log(Math.pow(tokenEncodingAlphabet.length, tokenSize)) / Math.log(256)) + 0.5);
        BigInteger buffer = new BigInteger(1, random.generateSeed(numberOfBytes - 3));
        ...
        while (generatedValue.length() < tokenSize) {
            if (buffer.compareTo(BigInteger.ZERO) == 0) {
                throw new IllegalStateException("Unexpected: Token could not be created.");
            }
